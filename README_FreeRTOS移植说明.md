...
- **风扇 PWM 调速**：环境温度超过阈值（默认 30℃）时分级调速——
  `≤阈值` 停转 / `阈值~阈值+5℃`（30~35℃）**50%** / `>阈值+5℃`（35℃以上）**100%**；
  TIM3_CH3/PB0 输出 **25kHz** PWM（人耳不可闻，消除 1kHz 时的电机啸叫）；
  手动模式沿用 APP 开关（开=100%）。
- **土壤湿度状态显示**：低于阈值=过低，阈值~阈值+20=正常，高于阈值+20=过高
  （默认阈值 60 → 即 <60 过低 / 60~80 正常 / >80 过高，随 APP/按键设置联动）；
  土壤读数 0（传感器开路/异常）时数值与状态均显示 `--`，
  温度/湿度/光照等其余各项照常显示——土壤传感器故障**仅显示异常、不再进入 Error 态**，
  不再出现 ErrorADC/Retry（仅 WiFi/云/OLED 等系统错误才显示全屏错误页）。
